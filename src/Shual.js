...
export const Shual = function( msg, salt, len = 8 ){
	...
	for( let j = msgs[0] ; j < len * 2 + msgs[0] ; j += 2 ){
		for( let i = 0 ; i < msgs.length * 3 ; i+=3 ){
			...
			let fib = [...Fibonacci( fibFactor )].reverse().slice( 0, 64 ).map( f => [ f.charAt(index1), f.charAt(index2), f.charAt(index3), f.charAt(index4) ] ).flat();
	...
	let bts = Split( res, 3 ).map( s => Add( Mod(Add(s,0),95), 32 ) );
